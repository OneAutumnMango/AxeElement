...
{
    public class BloodSecondary : Spell
    {
        // Degrees each blade is offset left/right from the aim direction.
        private const float SPREAD_ANGLE = 35f;

        // Degrees per FixedUpdate (50 Hz default) that each blade curves inward.
        // At 50 Hz, 1.5°/frame × 50 frames (1.0 s) ≈ 75° total arc, which
        // sweeps each blade back from the spread angle to center over the flight.
        ...
        {
            Plugin.Log.LogInfo($"[BloodSecondary] Initialize: owner={identity?.owner}, pos={position}, vel={this.initialVelocity}");
            try
            {
                float vel = this.initialVelocity;
                int owner = identity.owner;

                // ── Left blade: offset +SPREAD_ANGLE, arcs right (inward) ──
                Quaternion leftRot = rotation * Quaternion.Euler(0f, SPREAD_ANGLE, 0f);
                SpawnBlood(owner, position, leftRot, -ARC_RATE, vel);

                // ── Right blade: offset -SPREAD_ANGLE, arcs left (inward) ──
                Quaternion rightRot = rotation * Quaternion.Euler(0f, -SPREAD_ANGLE, 0f);
                SpawnBlood(owner, position, rightRot, +ARC_RATE, vel);

                Plugin.Log.LogInfo("[BloodSecondary] Both blades spawned successfully");
            }
            catch (Exception ex)
            {
                Plugin.Log.LogError($"[BloodSecondary] Initialize FAILED: {ex}");
            }
        }

        private static void SpawnBlood(int owner, Vector3 position, Quaternion rotation, float arcRate, float velocity)
        {
            ...
            var comp = go.AddComponent<BloodSecondaryObject>();
            comp.impact = _impact;
